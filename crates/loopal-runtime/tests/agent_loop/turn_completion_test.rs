...
impl Tool for FakeCompletionTool {
    ...
    async fn execute(&self, input: serde_json::Value, _ctx: &ToolContext,
        ...
        let r = input.get("result").and_then(|v| v.as_str()).unwrap_or("done");
...
fn make_multi_runner(
    ...
    if register_completion { kernel.register_tool(Box::new(FakeCompletionTool)); }
    let session = Session {
        id: "test-multi".into(), title: "".into(),
...
async fn test_attempt_completion_exits_turn_immediately() {
    let calls = vec![vec![
        Ok(StreamChunk::ToolUse {
            id: "tc-1".into(), name: "AttemptCompletion".into(),
...
async fn test_tool_then_completion_two_llm_calls() {
    ...
    let calls = vec![
        ...
        vec![
            Ok(StreamChunk::ToolUse {
                id: "tc-2".into(), name: "AttemptCompletion".into(),
...
async fn test_error_preserves_prior_output() {
    let calls = vec![
        vec![
            Ok(StreamChunk::Text { text: "first output".into() }),
            Ok(StreamChunk::Done { stop_reason: StopReason::EndTurn }),
        ],
...
async fn test_stream_error_after_tool_preserves_last_text() {
    ...
    let calls = vec![
        ...
        vec![
            Ok(StreamChunk::Text { text: "I will read the file.".into() }),
            Ok(StreamChunk::ToolUse {
                id: "tc-1".into(), name: "Read".into(),
                input: serde_json::json!({"file_path": tmp.to_str().unwrap()}),
            }),
